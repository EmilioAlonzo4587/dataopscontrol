...
async def top_slow_queries(
    ...
    """Top 10 slowest queries by average duration — for BI dashboard."""
    result = await db.execute(
        select(QueryLog)
        .where(QueryLog.category.in_([QueryCategory.SLOW, QueryCategory.CRITICAL]))
...
async def seed_demo_queries(db: AsyncSession = Depends(get_db)):
    ...
    sample_queries = [
        ...
        ("SELECT * FROM products WHERE LIKE '%gadget%'", 1500),
